...
void update_ice_pic_collision(GameData* game, Entity* icePicCollision, float delta_t) {
	...
	bool* exploding = get(icePicCollision->objects, "is_exploding", strcmp);
	if (exploding == NULL) {
        return;
	}
	printf("exploding : %i \n", *exploding);
	...
	if (*exploding) {
		damage_entity(game, icePicCollision, *damage, 1000, -1, false, icePicCollision);
	}

	if (are_colliding(icePicCollision->hit_box, game->player->hurt_box)) {
		damage_entity(game, game->player, *damage, 1000, -1, false, icePicCollision);
		*exploding = true;
    }
